@adminEnd2End @regression  @payMultiOrderWithExt @salesPage @prod @prodSmoke
Feature: net30
In order for a user with admin credentials to add a net order and process a batch payment
...
then the user can save orders in the net order based on the net days and process to processing the payment
...
@pp
Scenario: add order to a net/multi order and batch pay
Given i navigate to the netmulti order page
When i search for the net client "NewClient TestingTesting"
And i click on the auto net order created "NewClient TestingTesting"
